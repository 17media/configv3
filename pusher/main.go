...
func Pusher(etcdConn *clientv3.Client, root, etcdRoot string) {
	...
	if err != nil {
		...
		jerr := json.Unmarshal([]byte(resp.Kvs[0].Value), prevCFG)
	...
	for _, mod := range *modFiles {
		...
		_, err = etcdConn.Delete(ctx, fileP, clientv3.WithPrefix())
		if err != nil {
	...
	_, err = etcdConn.Put(ctx, infoPath, string(jsonBytes))
	if err != nil {
...
func main() {
	...
	etcdConn, err := clientv3.New(clientv3.Config{
		Endpoints:   strings.Split(ensembleStr, ","),
